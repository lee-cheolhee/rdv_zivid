...
namespace
{
    ...
    public:
        ...
        std::string rosTypeName()
        {
            ...
            else if constexpr (std::is_same_v<RosType, Zivid::Range<double>>
                || std::is_same_v<RosType, Zivid::PointXYZ>)
            {
                return "double_t";
            }
            else
        ...
        void apply(const SettingsNode& node)
        {
            ...
            else
            {
                ...
                else if constexpr (IsEnumSetting<SettingsNode>::value)
                {
                    ...
                    ss_ << ", " << min_index << ", " << max_index << ", edit_method="
                        << rosGeneratedEnumVariableName(setting_name);
                }
                ss_ << ")\n";
            }
        }
        ...
        std::string str()
        {
            ...
            {
                res
                    << "gen.add(\"enabled\", bool_t, 0, \"When this acquisition is enabled it will be included in captures\", "
                       "False)\n";
            }
            res << ss_.str();
    ...
    {
    public:
        ApplyConfigToZividSettingsGenerator(const std::string& config_class_name)
        ...
        void apply(const SettingsNode& node)
        {
            ...
            {
                ss_ << "static_cast<float>(" << cfg_id << "_x), static_cast<float>(" << cfg_id
        ...
        std::string str()
        {
            ...
            std::stringstream res;
            res << "inline static void applyConfigToZividSettings(const zivid_camera::" << config_class_name_
                << "Config& cfg, "
                << root_type_fq << "& s)\n";
            res << "{\n";
            res << ss_.str();
    ...
    public:
        ConfigUtilsHeaderGenerator(const std::string& config_class_name)
            : apply_config_zivid_settings_gen(config_class_name), zivid_settings_to_config_gen(config_class_name),
    ...
    public:
        Generator(const std::string& config_class_name)
            : config_class_name_(config_class_name), dynamic_reconfigure_cfg_gen_(config_class_name),
    ...
    void traverseSettingsTree(const SettingsNode& node, GeneratorType& generator)
    {
        ...
        {
            node.forEach([&](const auto& child)
            { traverseSettingsTree<SettingsType>(child, generator); });
        }
        else
        {
