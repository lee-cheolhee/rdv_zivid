...
namespace
{
    ...
    ZividSettingsType fillInUnsetWithCameraDefault(const ZividSettingsType& settings,
...
namespace zivid_camera
{
    ...
    public:
        ConfigDRServer(const std::string& name, ros::NodeHandle& nh, const Zivid::Camera& camera)
            : name_(name), dr_server_(dr_server_mutex_, ros::NodeHandle(nh, name_)),
              config_(ConfigType::__getDefault__())
        {
            static_assert(
                std::is_same_v<ZividSettings, Zivid::Settings> || std::is_same_v<ZividSettings, Zivid::Settings2D> ||
    ...
    template<typename ZividSettingsType, typename SettingsConfigType, typename SettingsAcquisitionConfigType>
    ZividSettingsType
    CaptureSettingsController<ZividSettingsType,
                              SettingsConfigType,
                              SettingsAcquisitionConfigType>::zividSettings() const
    {
    ...
    template<typename ZividSettingsType, typename SettingsConfigType, typename SettingsAcquisitionConfigType>
    void CaptureSettingsController<ZividSettingsType,
                                   SettingsConfigType,
                                   SettingsAcquisitionConfigType>::setZividSettings(
        const ZividSettingsType& settings)
    {
        ...
        if (numAcquisitions > numAcquisitionConfigServers())
        {
            ...
            error << "The number of acquisitions (" + std::to_string(numAcquisitions) + ") "
                  << "is larger than the number of dynamic_reconfigure " + config_node_name_
                      + "/acquisition_<n> servers ("
