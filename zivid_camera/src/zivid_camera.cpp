...
namespace zivid_camera
{
    ZividCamera::ZividCamera(ros::NodeHandle& nh, ros::NodeHandle& priv)
        : nh_(nh), priv_(priv), camera_status_(CameraStatus::Idle), use_latched_publisher_for_points_xyz_(false),
          use_latched_publisher_for_points_xyzrgba_(false), use_latched_publisher_for_color_image_(false),
          use_latched_publisher_for_depth_image_(false), use_latched_publisher_for_snr_image_(false),
          use_latched_publisher_for_normals_xyz_(false), image_transport_(nh_), header_seq_(0)
    {
        ...
        priv_.param<bool>("update_firmware_automatically",
        ...
        else
        {
            ...
            if (!Zivid::Firmware::isUpToDate(camera_))
            {
                ...
                {
                    ROS_INFO(
                        "The camera firmware is not up-to-date, and update_firmware_automatically is true, starting update");
                ...
                {
                    ROS_ERROR(
                        "The camera firmware is not up-to-date, and update_firmware_automatically is false. Throwing error.");
        ...
        depth_image_publisher_ =
            image_transport_.advertiseCamera("depth/image", 1, use_latched_publisher_for_depth_image_);
        ...
        load_settings_2d_from_file_service_ =
            nh_.advertiseService("load_settings_2d_from_file",
                &ZividCamera::loadSettings2DFromFileServiceHandler,
                this);
        ...
        CaptureAssistantSuggestSettings::Response&)
    {
        ...
        const auto max_capture_time =
            std::chrono::round<std::chrono::milliseconds>(std::chrono::duration<double>{
                req.max_capture_time.toSec() });
        ...
        SuggestSettingsParameters
            suggest_settings_parameters{ SuggestSettingsParameters::MaxCaptureTime{ max_capture_time },
    ...
    void ZividCamera::publishFrame(const Zivid::Frame& frame)
    {
            ...
            publish_normals_xyz)
        {
            ...
            const auto
                transformationMatrix = Zivid::Matrix4x4{ scale, 0, 0, 0, 0, scale, 0, 0, 0, 0, scale, 0, 0, 0, 0, 1 };
    ...
    void ZividCamera::publishColorImage(const std_msgs::Header& header,
    ...
    void ZividCamera::publishDepthImage(const std_msgs::Header& header,
        const sensor_msgs::CameraInfoConstPtr& camera_info,
        const Zivid::PointCloud& point_cloud)
    {
    ...
    void ZividCamera::publishSnrImage(const std_msgs::Header& header,
        const sensor_msgs::CameraInfoConstPtr& camera_info,
        const Zivid::PointCloud& point_cloud)
    {
