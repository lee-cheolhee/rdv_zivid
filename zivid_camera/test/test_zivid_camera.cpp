...
protected:
    ...
    public:
        static SubscriptionWrapper<Type> make(ros::NodeHandle& nh, const std::string& name)
        {
            ...
            boost::function<void(const boost::shared_ptr<const Type>&)>
                cb = [impl = w.impl_.get()](const auto& v) mutable
            {
              impl->num_messages_++;
    ...
    {
        dynamic_reconfigure::Client<zivid_camera::SettingsAcquisitionConfig>
            acquisition_0_client("/zivid_camera/settings/"
                                 "acquisition_0/");
...
protected:
    ...
    void testGeneralMinMaxDefault(const std::string& service_name)
    {
        ...
        if constexpr (std::is_same_v<ZividSettingsType, Zivid::Settings>)
        {
            ...
            ASSERT_EQ(default_cfg.processing_filters_outlier_removal_threshold,
                sdkDefaultValue<OutlierRemovalThreshold>());
            ASSERT_EQ(min_cfg.processing_filters_outlier_removal_threshold,
                sdkValidRange<OutlierRemovalThreshold>().min());
            ASSERT_EQ(max_cfg.processing_filters_outlier_removal_threshold,
                sdkValidRange<OutlierRemovalThreshold>().max());
        }
    }
    ...
    void testAcquisitionMinMaxDefault(const std::string& prefix, std::size_t num_acquisition_servers)
    {
        ...
        ASSERT_FALSE(ros::service::waitForService(
            prefix + "acquisition_" + std::to_string(num_acquisition_servers) + "/set_parameters",
            short_wait_duration));
    }
};
...
TEST_F(DynamicReconfigureMinMaxDefaultTest, testDynamicReconfigureSettingsMinMaxDefaultValue)
{
    ...
    testAcquisitionMinMaxDefault<Zivid::Settings, zivid_camera::SettingsAcquisitionConfig>("/zivid_camera/settings/",
        10);
...
protected:
    ...
    void compareSettingsWithNodeState(const ZividSettingsType& settings)
    {
        ...
        {
            compareSettingsAcquisitionConfigWithSettings(acquisitions[i],
                settingsAcquisitionConfig<ZividSettingsType>(i));
        }
...
private:
    ...
    void compareSettingsConfigWithSettings(const Zivid::Settings& s, const zivid_camera::SettingsConfig& cfg) const
    {
        ...
        ASSERT_EQ(filters.smoothing().gaussian().isEnabled().value(),
            cfg.processing_filters_smoothing_gaussian_enabled);
        ...
        ASSERT_EQ(filters.reflection().removal().isEnabled().value(),
            cfg.processing_filters_reflection_removal_enabled);
    }
...
private:
    ...
    static void recursivelyFillInUnsetWithCameraDefault(Node& node, const Zivid::CameraInfo& cameraInfo)
    {
        ...
        {
            node.forEach([&cameraInfo](auto& child)
            { recursivelyFillInUnsetWithCameraDefault(child, cameraInfo); });
        }
...
protected:
        ...
        zivid_camera::CaptureAssistantSuggestSettings::Request::_ambient_light_frequency_type ambient_light_frequency)
    {
        ...
        throw std::runtime_error(
            "Could not convert value " + std::to_string(ambient_light_frequency) + " to API enum.");
    }
...
TEST_F(ZividCATest, testDifferentMaxCaptureTimeAndAmbientLightFrequency)
{
    ...
    {
        for (auto ambient_light_frequency : { Request::AMBIENT_LIGHT_FREQUENCY_NONE,
                                              Request::AMBIENT_LIGHT_FREQUENCY_50HZ,
